...
struct node*head=NULL; void insertfirst()
{
struct node*newnode;
...
newnode->next=NULL; if(head==NULL) head=newnode;
else
{
...
temp=head; if(newnode==NULL) printf("\n no space"); else
{
...
struct node*temp,*newnode;
newnode=(struct node*)malloc(sizeof(struct node*)); newnode->next=NULL;
printf("\n enter the value of node after new node to be inserted :"); scanf("%d",&key);
temp=head;
while(temp->data!=key)
{
temp=temp->next; if(temp==NULL) break;
}
{
printf("The %d value not exist",key); return;
}
if(temp->data==key)
...
void deletefirst()
{
struct node*temp; temp=head; head=temp->next;
printf("%d deleted",temp->data); free(temp);
}
...
void deletelocation()
{
struct node*temp,*p; int key;
printf("\n enter the value of node to be deleted :"); scanf("%d",&key);
temp=head;
while(temp->data!=key)
{
p=temp; temp=temp->next; if(temp==NULL) break;
}
if(temp==NULL)
...
if(temp->data==key)
{
printf("%d deleted",temp->data); if(temp==head)
{
p=head; head=temp->next; free(p);
}
else
{
...
struct node*temp; int key,pos=0;
...
void display()
{
struct node*p; if(head==NULL) printf("list is empty");
...
case 9:exit(0);
printf("exiting the program "); return;
default:printf("\n invalid option");
}
}
